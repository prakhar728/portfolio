  ...
  (
    ...
    const suggestions = React.useMemo(() => {
      ...
      if (!messages.length && initialSuggestions.length > 0) {
        return initialSuggestions.slice(0, maxSuggestions).filter((s) => s.detailedSuggestion?.trim());
      }
      // Otherwise use generated suggestions (filter out any with empty content)
      return generatedSuggestions.filter((s) => s.detailedSuggestion?.trim());
    }, [
      messages.length,
        ...
        ? suggestions.map((suggestion, index) => (
            ...
            >
              <button
                ...
                onClick={async () => {
                  if (!isGenerating && suggestion.detailedSuggestion?.trim()) {
